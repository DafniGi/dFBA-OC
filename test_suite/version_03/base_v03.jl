...
# number of reactions
    ...
    @NLobjective(m, Max, +
        N[4,end,end]/sum(tCI[i] for i in 1:nCI) * ##PHI## -
        sum(
            sum(
                - phi1*FO_L[mc,i] 
    ...
    @NLexpressions(m, begin
        ...
        q_X[i=1:nCI], log((-q_G[i]-qmax+2qmin)/(q_G[i]-qmax))*1/Kup
    end)
    
    @NLconstraints(m, begin
        # fix ATPM in FBA
        NLc_q_M1[i=1:nCI],   q[atp,i] - q_M[i] == 0
        # fix q_G in FBA
        NLc_q_G1[i=1:nCI], - q[glu,i] - q_G[i] == 0 # |q_G| >= |q[glu]| [mmol/(g h)]
        # fix biomass as function of q_G in FBA
        NLc_q_G2[i=1:nCI],   q[xxx,i] - q_X[i] == 0
        end)
    @constraints(m, begin
    ...
    @constraints(m, begin
        # glucose
        m1[i=1:nCI, j=1:nCP], Ndot[1,i,j] == F[i]*c_G + q[glu,i]*N[2,i,j]
        # biomass
        m2[i=1:nCI, j=1:nCP], Ndot[2,i,j] == q[xxx,i]*N[2,i,j]
        # sulfate
